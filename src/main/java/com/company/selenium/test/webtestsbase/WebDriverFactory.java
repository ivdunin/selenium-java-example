...
public class WebDriverFactory {
    ...
    public static void startBrowser(boolean isLocal) {
        String geckodriverPath = System.getenv("GECKO_DRIVER");
        if (driver == null) {
            ...
            if (!isLocal) {
                ...
                switch (browser) {
                    case FIREFOX:
                        ...
                        {
                            // TODO: check how to setup geckodriver path in Jenkins
                            // Execute only if cannot run firefox based on PATH envs
                            if (SystemUtils.IS_OS_LINUX)
                                System.setProperty("webdriver.gecko.driver", TestsConfig.getConfig().getGeckodriverLinux());
                            else if (SystemUtils.IS_OS_WINDOWS)
                                System.setProperty("webdriver.gecko.driver", TestsConfig.getConfig().getGeckodriverWin());
                            else
                                throw new IllegalStateException(String.format("Unknown OS type: %s! Cannot set geckodriver path!", SystemUtils.OS_NAME));
                            driver = new FirefoxDriver(CapabilitiesGenerator.getDefaultCapabilities(Browser.FIREFOX));
                        }
                        break;
                    case CHROME:
    ...
    public static void takeScreenShot() {
        ...
        System.out.println("Create error screenshot!");
        String curDir = System.getProperty("user.dir");
        File scrFile = ((TakesScreenshot)driver).getScreenshotAs(OutputType.FILE);
        try {

            String screenshotFilename = String.format("screenshot-%s.png", dateFormat.format(date));
            FileUtils.copyFile(scrFile, new File(Paths.get(curDir, "screenshoots", screenshotFilename).toString()));
            System.out.println(String.format("Save screenshot to file: %s", screenshotFilename));
        } catch (IOException e)
        {
            System.out.println("ERROR: Cannot save screenshot!");
